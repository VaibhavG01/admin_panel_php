...
<?php require "middleware/admin.php"; ?>
  <div class="p-6 max-w-lg mx-auto bg-white shadow-md rounded-xl">
    <h2 class="text-2xl font-bold text-slate-800 capitalize mb-6">
      <?php echo isset($pageTitle) ? htmlspecialchars($pageTitle) : 'Add Banner'; ?>
    </h2>
    <form method="POST" action="index.php?route=banner_store" enctype="multipart/form-data" class="space-y-6">
      <div>
        <label for="title" class="block text-sm font-semibold text-slate-700 mb-2">Banner Title</label>
        <input type="text" name="title" id="title" placeholder="Enter banner title" required
      ...
      <div>
        <label for="banner_image" class="block text-sm font-semibold text-slate-700 mb-2">Banner Image</label>
