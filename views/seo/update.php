...
<?php require "middleware/admin.php"; ?>
    ...
    <form method="POST" action="index.php?route=seo_update&id=<?= $seo['id'] ?>" class="space-y-6">
        <div>
            <label for="meta_tags" class="block text-sm font-semibold text-slate-700 mb-2">Meta Tags</label>
            <input type="text" name="meta_tags" id="meta_tags" value="<?= htmlspecialchars($seo['meta_tags']) ?>" required
        ...
        <div>
            ...
            <textarea name="meta_descriptions" id="meta_descriptions" rows="4" required
                class="w-full px-4 py-2 text-sm text-slate-700 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:border-fuchsia-500 transition-all"><?= htmlspecialchars($seo['meta_descriptions']) ?></textarea>
        </div>

        <div>
            <label class="flex items-center">
                <input type="checkbox" name="isActive" <?= $seo['isActive'] ? "checked" : "" ?>
        ...
        <div>
            <button type="submit"
                ...
                <i class="fa fa-save mr-2"></i> Update SEO
            </button>
        </div>
    </form>
</div>
