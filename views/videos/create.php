...
<?php require "middleware/admin.php"; ?>
  <div class="p-6 max-w-lg mx-auto bg-white shadow-md rounded-xl">
    <h2 class="text-2xl font-bold text-slate-800 capitalize mb-6">
      <?php echo isset($pageTitle) ? htmlspecialchars($pageTitle) : 'Add videos'; ?>
    </h2>
    <form method="POST" action="index.php?route=videos_store" enctype="multipart/form-data" class="space-y-6">
      ...
      <div>
        <label for="videos" class="block text-sm font-semibold text-slate-700 mb-2">Video File</label>
        <input type="file" name="videos" id="videos" required
      ...
      <div>
        <button type="submit"
          ...
          <i class="fa fa-save mr-2"></i> Save Video
        </button>
      </div>
    </form>
  </div>
